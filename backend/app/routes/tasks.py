...
def get_tasks():
    ...
    docs = list(db[TaskModel.COLLECTION].find(query).sort("due_date", 1))
...
def create_task():
    ...
    result = db[TaskModel.COLLECTION].insert_one(doc)
...
def update_task(task_id):
    ...
    try:
        result = db[TaskModel.COLLECTION].update_one(
...
def delete_task(task_id):
    try:
        result = db[TaskModel.COLLECTION].delete_one({"_id": ObjectId(task_id)})
    except InvalidId:
        return jsonify({"error": "Invalid ID"}), 400
